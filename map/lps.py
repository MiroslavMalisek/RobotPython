...
class LPS(TkMap):
   ...
   """
    ...
    def bayesianCertainty(
        self, distMM, angle, distHitMM, arc, rangeSonarMM, resolutionMM
    ):
        ...
        certaintyFactor = (
            (float(rangeSonarMM - distMM) / rangeSonarMM) + (float(arc - angle) / arc)
        ) / 2
    ...
    def computeOccupancy(
        self, origx, origy, hitx, hity, arc, rangeSonarMM, resolutionMM, label=None
    ):
        """
      ...
      """
        ...
        if self.debug:
            print("occupancyGrid:", (origx, origy, hitx, hity))
        ...
        if abs(run) < 0.1:
            run = 0
        steps = int(
            math.ceil(max(abs(rise / self.colScaleMM), abs(run / self.rowScaleMM)))
        )

        if steps == 0:
            # set the origin of the sensor we are using to be occuped
            self.setGridLocation(row=hitx, col=hity, value=1.0, label=label, absolute=0)
            return
        ...
        for step in range(steps):
            ...
            distToCurr = math.sqrt(
                math.pow(currx - origx, 2) + math.pow(curry - origy, 2)
            )
            ...
            while currentOffset < radiusAtCurr:
                ...
                newValue = self.bayesianCertainty(
                    distMM=distToCurr,
                    angle=angle,
                    distHitMM=distToHit,
                    arc=arc,
                ...
                currentOffset = math.sqrt(
                    math.pow(currentOffsetX, 2) + math.pow(currentOffsetY, 2)
                )
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                self.canvas.create_rectangle(
                    ...
                    width=0,
                    fill=self.color(
                        self.getGridLocation(row=row, col=col, absolute=1), maxval
                    ),
                    tag="cell%d" % self.newCellLabel,
                )
                if drawLabels and self.label[row][col]:
                    self.canvas.create_text(
                        int((x + 0.5) * self.colScale),
                        int((y + 0.5) * self.rowScale),
