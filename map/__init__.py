...
class Map:
    ...
    def reset(self, value=0.5):
        self.grid = [[value for col in range(self.cols)] for row in range(self.rows)]
        self.label = [["" for col in range(self.cols)] for row in range(self.rows)]
    ...
    def getAbsFromRel(self, row, col):
        return (
            int(round((self.originMM[1] - row) / self.rowScaleMM)),
    ...
    def validateGrid(self):
        print(
            "Validating Grid: Checking bounds (%d, %d)..." % (self.rows, self.cols),
            end=" ",
        )
