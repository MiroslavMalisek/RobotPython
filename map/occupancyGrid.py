...
try:
    ...
    print("PIL not available, unable to load Map Images.")
...
class OccupancyGrid(TkMap):
   ...
   """
    ...
    def __init__(
            ...
            try:
                ...
                if gridResize != 1.0:
                    ...
                    self.image = self.image.resize(
                        (frameWidth, frameHeight), Image.BICUBIC
                    )
        ...
        self.threshhold = 0.3
        self.distanceToGoal = [
            [self.infinity for col in range(self.cols)] for row in range(self.rows)
        ]
    ...
    def eventToCell(self, event):
        return (
            int(round(event.x / self.colScale)),
    ...
    def setDouble(self, event):
        ...
        self.grid = [[0.0 for col in range(self.cols)] for row in range(self.rows)]
        self.redraw(self.grid)
    ...
    def findPath(self, event=None):
        ...
        if self.debug:
            print("Finding path...")
        self.calculateAllCosts(
            goalCol=self.getGoalCol(),
            goalRow=self.getGoalRow(),
            maxIters=self.rows * self.cols,
        )
        (pathGrid, pathList) = self.getPath(
            row=self.getCurrentRow(), col=self.getCurrentCol()
        )
        if pathGrid:
            print("Done!")
    ...
    def tooTight(self, row, col, i, j):
        ...
        return (
            self.distanceToGoal[row + i][col] == self.infinity
            or self.distanceToGoal[row][col + j] == self.infinity
        )
                            ...
                            for j in [-1, 0, 1]:
                                if self.inRange(row + i, col + j):
                                    if i == 0 and j == 0:
                                        ...
                                        d = 1.00
                                    newDist = self.distanceToGoal[row + i][col + j] + d
                                    if newDist < self.distanceToGoal[row][col]:
                                        valuesChanged += 1
                                        self.distanceToGoal[row][col] = newDist
    ...
    def getPath(self, row, col):
        ...
        while not (self.distanceToGoal[row][col] == 0.0):
            ...
            for i in [-1, 0, 1]:
                for j in [-1, 0, 1]:
                    if self.inRange(row + i, col + j):
                        if self.distanceToGoal[row + i][
                            col + j
                        ] < bestStep and not self.tooTight(row, col, i, j):
   ...
   """
    ...
    def getProbKonolige(self, prevVal, probOcc=1, certainty=0):
        ...
        return math.log((prevValInv * probOcc), 10) * math.log(
            (prevVal * probEmpty), 10
        )
    ...
    def getRelativePosInOtherGrid(
        self, row, col, oGrid, rowOffset, colOffset, angle, axisRow, axisCol
    ):
        ...
        row = (
            (math.cos(angle) * (translatedCellRow - axisRow))
            - (math.sin(angle) * (translatedCellCol - axisCol))
            + axisRow
        )
        ...
        # with (x0,y0) the axis and (x1,y1) the cell to translate
        col = (
            (math.sin(angle) * (translatedCellRow - axisRow))
            + (math.cos(angle) * (translatedCellCol - axisCol))
            + axisCol
        )
   ...
   """
    ...
    def mergeOtherGridInfo(
        ...
        res = otherGrid.calculateTransformation(
            oGrid=self,
        ...
        for row in range(otherGrid.rows):
            for col in range(otherGrid.cols):
                ...
                cellTransf = otherGrid.getRelativePosInOtherGrid(
                    row=row,
                    col=col,
                    oGrid=self,
                    rowOffset=res[0],
                ...
                if (
                    cellTransfRow >= 0
                    and cellTransfRow < self.rows
                    and cellTransfCol >= 0
                    and cellTransfCol < self.cols
                ):

                    myCurrentValue = self.getGridLocation(
                        row=cellTransfRow, col=cellTransfCol, absolute=1
                    )
                    otherCurrentValue = otherGrid.getGridLocation(
                        row=row, col=col, absolute=1
                    )
