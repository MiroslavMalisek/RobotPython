...
class TKgui(tkinter.Toplevel, gui):
    def __init__(self, engine):
        ...
        menu = [
            ...
            ("Help", [["Help", self.help], ["About", self.about],]),
        ]
        ...
        for entry in menu:
            self.mBar.add_cascade(
                label=entry[0], menu=self.makeMenu(self.mBar, entry[0], entry[1])
            )
        ...
        # Display:
        self.loadables = [
            ("button", "Server:", self.loadSim, self.editWorld, 0),  # 0 = False
    ...
    def watchCallback(self, full_id):
        if len(full_id) == 3 and (
            full_id[0] == "robot"
            and full_id[1] in self.engine.robot.devices
            and type(full_id[2]) == type(0)
        ):
    ...
    def getTreeContents(self, node, tree):
        ...
        for item in node.full_id()[1:]:
            if item == "methods":  # methods
                for method in dir(thing):
                    if (
                        method[0] != "_" or method[1] == "_"
                    ) and method not in thing.__dict__:
                        ...
                        if object == None:
                            ...
                            tree.add_node(
                                "%s = '%s'" % (method, object.strip()),
                                id=method,
                                flag=0,
                            )
                        elif type(object) in [float, int, bool, int, dict, list, tuple]:
                            tree.add_node(
                                "%s = %s" % (method, object), id=method, flag=0
                            )
                        else:
                            ...
                            if docString != None:
                                ...
                                tree.add_node(
                                    "%s(): %s" % (method, docString),
                                    id="%s()" % method,
                                    flag=0,
                                )
                            else:
                                tree.add_node(
                                    "%s()" % (method,), id="%s()" % method, flag=0
                                )
        ...
        if type(thing) == type([]):  # list:
            ...
            if len(thing) == 0 or (
                len(thing) > 0 and not issubclass(type(thing[0]), Device)
            ):
            ...
            for item in dictkeys:
                if item[0] == "_":
                    ...
                    if "devices" in thing.__dict__ and item in thing.devices:  # robot
                        ...
                        keys = list(thing.__dict__[item].keys())
                        keys.sort()
                        keysComma = ""
                        for key in keys:
                            if keysComma:
                        ...
                        tree.add_node(
                            "%s = '%s'" % (item, thing.__dict__[item]), id=item, flag=0
                        )
    ...
    def makeRobotTree(self):
        if self.engine and self.engine.robot:
            if self.robotTreeWindow:
                ...
                self.robotTreeWindow = TreeWindow(
                    share.gui,
                    "robot",
                    self.getTreeContents,
                    self.watchCallback,
    ...
    def makeBrainTree(self):
        if self.engine and self.engine.brain:
            if self.brainTreeWindow:
                ...
                self.brainTreeWindow = TreeWindow(
                    share.gui,
                    "brain",
                    self.getTreeContents,
                    self.watchCallback,
    ...
    def makeRow(self, item):
        ...
        if type == "button":
            self.buttonArea[load] = tkinter.Button(
                tempframe, text=load, width=10, command=loadit, state="disabled"
            )
            self.textArea[load] = tkinter.Button(
                tempframe, command=editit, justify="right", state="disabled"
            )
            if viewit:
                self.buttonArea["View " + load] = tkinter.Button(
                    tempframe, text="View", width=10, command=viewit, state="disabled"
                )
                self.buttonArea["View " + load].pack(
                    side=tkinter.RIGHT, fill="none", expand="no", anchor="n"
                )
        elif type == "status":
        ...
        self.buttonArea[load].pack(
            side=tkinter.LEFT, fill="none", expand="no", anchor="n"
        )
    ...
    def CommandPreviousKey(self, event):
        if (
            self.history_pointer - 1 <= len(self.history)
            and self.history_pointer - 1 >= 0
        ):
    ...
    def CommandTabKey(self, event):
        ...
        self._populateEnv()
        try:
            exec(("_methods = %s.__doc__" % command), self.environment)
        ...
        succeed = 0
        try:
            exec(("_methods = dir(%s)" % command), self.environment)
            succeed = 1
        except:
            pass
        if succeed:
            ...
            for m in methods:
                ...
                succeed = 1
                try:
                    exec(("_methods = type(%s.%s)" % (command, m)), self.environment)
                except:
                    succeed = 0
        ...
        if cnt % 2 != 1:
            print()
        print("----------------------------------------------------------")
        if "_methods" in self.environment:
            del self.environment["_methods"]
    ...
    def update(self):
        ...
        needToUpdateState = 1
        try:
            needToUpdateState = self.engine.brain.needToStop
        except:
            pass
        ...
        if self.engine.robot != 0:
            if self.engine.robot.stall:
                ...
                bump = ""
            try:
                self.textArea["Pose:"].config(
                    text="X: %4.2f Y: %4.2f Th: %4.0f  %s"
                    % (
                        self.engine.robot.x,
                        self.engine.robot.y,
        ...
        # Enable and Disable textAreas and their buttons.
        ...
        if self.textArea["Robot:"]["text"]:  # have a robot!
            self.mBar.entryconfig("Robot", state="normal")
            ...
            self.mBar.entryconfig("Robot", state="disabled")
            if self.buttonArea["Brain:"]["state"] != "disabled":
        ...
        if self.textArea["Brain:"]["text"]:  # have a brain!
            self.mBar.entryconfig("Brain", state="normal")
            ...
            self.mBar.entryconfig("Brain", state="disabled")
            if self.goButtons["Run"]["state"] != "disabled":
    ...
    def fileloaddialog(self, filetype, skel, startdir=""):
        ...
        try:
            ...
            print("failed to open file dialog! - ", e)
    ...
    def filesavedialog(self, filetype, skel, startdir="", default=None):
        ...
        if startdir == "":
            ...
            chdir(startdir)
        d = TKwidgets.SaveFileDialog(
            self, "Save " + filetype, skel, defaultFilename=default
        )
        try:
            if d.Show() == 1:
                doc = d.GetFileName()
    ...
    def disconnectRobot(self):
        if self.engine.robot != 0:
            ...
            raise Error("select robot first")
