   ...
   """
    ...
    def __init__(self, name="abstract gui", options={}, engine=0):
      ...
      """
        ...
        if file_exists(os.getenv("HOME") + "/.pyrobothist"):
            ...
            self.history = list(map(str.strip, fp.readlines()))
            fp.close()
    ...
    def run(self, command=[]):
      ...
      """
        ...
        print("https://github.com/dsblank/pyrobot")
    ...
    def setCurrentConfig(self, config):
        ...
        if self.engine.simfile:
            ...
            if pyrobotdir() + "/plugins/" in file or cwd in file:
    ...
    def processCommand(self, retval):
        ...
        if retval == "run":
            ...
            try:
                ...
                try:
                    exec(exp1, self.environment)
                except:
                    try:
                        exec(exp2, self.environment)
                    except:
    ...
    def loadBrain(self):
        ...
        if f != "" and f != 0:
            self.lastDir["brain"] = "/".join(f.split("/")[:-1])
            self.freeBrain()
    ...
    def loadDevice(self):
        ...
        if f != "" and f != 0:
            self.lastDir["devices"] = "/".join(f.split("/")[:-1])
    ...
    def loadSim(self, worldfile=""):
        ...
        if f != "" and f != 0:
            self.lastDir["sim"] = "/".join(f.split("/")[:-1])
            if worldfile == "":
                ...
                if simulatorName[-6:] == "Server":
                    ...
                    worldfile = self.fileloaddialog(
                        ...
                        self.lastDir.get(
                            ...
                            "%s/plugins/configs/%s/" % (pyropath, configDirName),
                        ),
                    )
                    if worldfile == "":
                        return
                    self.lastDir["%s-config" % simulatorName] = "/".join(
                        worldfile.split("/")[:-1]
                    )
                else:
                    # ends with "Simulator"
                    ...
                    if simulatorName == "PyrobotSimulator":
                        worldfile = self.fileloaddialog(
                            ...
                            self.lastDir.get(
                                ...
                                "%s/plugins/worlds/%s/" % (pyropath, simDirName),
                            ),
                        )
                    ...
                    if worldfile == "" or worldfile == 0:
                        return
                    self.lastDir["%s-world" % simulatorName] = "/".join(
                        worldfile.split("/")[:-1]
                    )
            else:
            ...
            if os.name in ["nt", "dos", "os2"]:
                ...
                raise Error("your OS (%s) is not supported" % os.name)

    def loadRobot(self):
        f = self.fileloaddialog("robots", "*.py", self.lastDir.get("robot", ""))
        if f != "" and f != 0:
            self.lastDir["robot"] = "/".join(f.split("/")[:-1])
            self.freeBrain()
            self.freeRobot()
