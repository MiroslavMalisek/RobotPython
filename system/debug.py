...
class PyrobotDebugger(code.InteractiveConsole):
    ...
    def displayTrace(self):
        ...
        for i in range(-len(self.frames) + 1, -3 + 1):
            ...
            if pointer == ">":
                print(
                    _colorize(
                        (" %s %2d) " + nameFormat + " at %s:%s")
                        % (pointer, c, c_es, a_es, b_es),
                        "green",
                    )
                )
            else:
    ...
    def displayHelp(self):
        print(
            "Commands:",
            _colorize("up, down, top, bot, help, quit, a frame number, edit,"),
        )
    ...
    def push(self, line):
        if line in ["up", "down", "top", "bot"]:
            if line == "up":
                ...
                self.currentPos = -3
            # ---------------- check and return
            if self.currentPos >= -len(self.frames) + 1 and self.currentPos <= -3:
                self.locals = self.frames[self.currentPos].f_locals
            else:
                self.currentPos = self.lastPos
            return
        if line in list(map(str, list(range(len(self.stack))))):
