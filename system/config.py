...
class Configuration:
    def __init__(self, file=None):
        self.data = {"robot": {}, "brain": {}, "simulator": {}, "pyrobot": {}}
    ...
    def processFile(self, file, cp):
        ...
        for sec in cp.sections():
            ...
            for opt in cp.options(sec):
                self.data[name][opt.lower()] = cp.get(sec, opt).strip()

    def save(self, file="pyrobot.ini"):
        fp = open(file, "w")
        for section in self.data:
            if section == "config":
                continue  # don't save config, that's what we're doing!
    ...
    def load(self, file=None):
        ...
        if file_exists(pyrobotdir() + "/.pyrobot-" + environ["HOSTNAME"]):
            # $PYRO-HOSTNAME?
            self.processFile(pyrobotdir() + "/.pyrobot-" + environ["HOSTNAME"], cp)
        if file_exists(pyrobotdir() + "/pyrobot-" + environ["HOSTNAME"] + ".ini"):
            # $PYRO-HOSTNAME?
            self.processFile(
