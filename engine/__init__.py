...
class Engine:
    ...
    def loadSimulator(self, file, worldfile):
        ...
        if simulatorName[-6:] == "Server":
            ...
            if system.file_exists(worldfile):
                pass  # leave it alone
            elif pyrobotdir() != None and system.file_exists(
                pyrobotdir() + "/plugins/configs/%s/%s" % (configDirName, worldfile)
            ):
                worldfile = pyrobotdir() + "/plugins/configs/%s/%s" % (
                    configDirName,
                    worldfile,
                )
            ...
            if system.file_exists(options[0]):
                ...
                pyrobotdir() + "/plugins/simulators/" + options[0]
            ):
                os.system(
                    pyrobotdir()
                    + "/plugins/simulators/"
            ...
            if system.file_exists(worldfile):
                ...
                pyrobotdir() + "/plugins/worlds/%s/%s" % (simDirName, worldfile)
            ):
                worldfile = pyrobotdir() + "/plugins/worlds/%s/%s" % (
                    simDirName,
                    worldfile,
                )
            if self.config.get("pyrobot", "gui").lower() == "tty":
    ...
    def loadRobot(self, file):
        ...
        if system.file_exists(file):
            ...
            pyrobotdir() + "/plugins/robots/" + file
        ):
            self.robot = system.loadINIT(pyrobotdir() + "/plugins/robots/" + file)
    ...
    def loadBrain(self, file):
        ...
        if system.file_exists(file):
            ...
            pyrobotdir() + "/plugins/brains/" + file
        ):
            try:
                self.brain.window.destroy()
            except:
            ...
            try:
                ...
                pass
            self.brain = system.loadINIT(pyrobotdir() + "/plugins/brains/" + file, self)
    ...
    def tryToConnect(self):
        ...
        if (self.robot == 0) or (self.brain == 0):
    ...
    def pleaseStep(self):
        ...
        if self.brain != 0:
            ...
            time.sleep(0.5)  # arbitrary time to allow it to do something
            self.robot.move(0, 0)
