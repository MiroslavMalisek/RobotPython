...
class Brain(threading.Thread):
    ...
    def run(self):
        ...
        while self.needToQuit != 1 and self.isAlive():
            # print "Acquire ----------------------------"
            count = 0
            ...
            if self.needToQuit:
                ...
                # print "wait()"
                self.condition.wait(0.25)  # FIX: .5?
                # print "release()"
                self.condition.release()
            ...
            if self.profilePerformance == 2:
                ...
                if self.profileCount % 100 == 0:
                    print(
                        "Profile: brain running at %.3f steps/second"
                        % (float(self.profileCount) / self.profileTotalTime)
                    )
    ...
    def pleaseRun(self, callback=0):
        ...
        if not self.isAlive():
            pyrobot.gui.console.log(
                pyrobot.gui.console.WARNING,
                "Brain thread is not alive but request to run was made.",
            )
        self.needToStop = 0
        if callback != 0:
            callback()
    ...
    def redraw(self):
        ...
        if getattr(self, "canvas", None) is not None:
            self.canvas.create_text(
                ...
                tags="pie",
                fill="black",
                text="This Brain needs a redraw method!",
            )
