"""
A wrapper for the TkSimulator to add a line
"""
...
class LineSimulation(TkSimulator):
    ...
    def __init__(self, dimensions, offsets, scale, root=None, run=1, background=None):
        TkSimulator.__init__(
            self,
        ...
        print(self.backgroundFile)

        if self.backgroundFile != None:
            if not os.path.exists(background):
                # we didn't get a valid file, let's try in the plugin dir
                self.backgroundFile = (
                    pyrobotdir() + "/plugins/worlds/Pyrobot/" + background
                )

            print(self.backgroundFile)
            try:
                self.backgroundImage = ImageTk.PhotoImage(
                    Image.open(self.backgroundFile), master=self
                )
                self.canvas.create_image(
                    0, 0, anchor="nw", image=self.backgroundImage, tags="background"
                )
            except:
    ...
    def getLineProperties(self):
        ...
        return self.symLine(
            math.trunc(self.scale_y(robot._gy)),
    ...
    def symLine(self, x, y, phi, delta, img):
       ...
       """
        ...
        p0 = np.array(
            [
                x + (math.sin(phi) * self.ROBOT_DIAMETER),
        ...
        pts = np.array(
            [
                (x, y)
                for x, y in ptsl
                if ((x > 1) and (y > 1) and (x < maxX - 2) and (y < maxY - 2))
            ]
        )
        weights = np.array(
            [
                w
                for (w, (x, y)) in zip(weights, ptsl)
                if ((x > 1) and (y > 1) and (x < maxX - 2) and (y < maxY - 2))
            ]
        )
        ...
        if len(pts) > 0:
            ...
            pts_greyvals = np.array([self.pixValBi(pt[0], pt[1], img) for pt in pts])
            ...
            if np.min(pts_greyvals) < self.BLACK_THRESHOLD:
                ...
                centroid = np.dot(weights, 255 - pts_greyvals) / np.sum(
                    255 - pts_greyvals
                )
