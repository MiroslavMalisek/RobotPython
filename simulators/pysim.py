...
class Segment:
    def __init__(self, start, end, id=None, partOf=None):
        ...
        if not self.vertical:
            ...
            self.yintercept = round(
                self.start[1] - self.start[0] * self.slope, RESOLUTION
            )

    def length(self):
        return math.sqrt(
            (self.start[0] - self.end[0]) ** 2 + (self.start[1] - self.end[1]) ** 2
        )
    ...
    def intersection(self, other):
        if self.parallel(other):
            ...
            return (other.start[0], self.yintercept + other.start[0] * self.slope)
        else:
            # m1x + b1 = m2x + b2; so
            # (m1 - m2)x + b1 - b2 == 0
            # (m1 - m2)x = b2 - b1
    ...
    def in_bbox(self, point):
        return (
            (self.end[0] <= round(point[0], RESOLUTION) <= self.start[0])
            or (self.start[0] <= round(point[0], RESOLUTION) <= self.end[0])
        ) and (
            (self.end[1] <= round(point[1], RESOLUTION) <= self.start[1])
            or (self.start[1] <= round(point[1], RESOLUTION) <= self.end[1])
        )
    ...
    def on_line(self, point):
        if self.vertical:
            ...
            return (
                round(point[0] * self.slope + self.yintercept, RESOLUTION)
                == round(point[1]),
                RESOLUTION,
            )
            ...
            for r in self.robots:
                ...
                if r.boundingBox != []:
                    ...
                    for i in range(len(xys)):
                        w = Segment(
                            xys[i], xys[i - 1]
                        )  # using the previous one completes the polygon
                        w.color = r.color
                        w.type = r.type
                        w.robot = r
                        segments.append(w)
                if r.boundingSeg != []:
                    ...
                    for i in range(0, len(xys), 2):
                        ...
                        w.robot = r
                        segments.append(w)
                for s in r.additionalSegments(
                    r._gx, r._gy, cos_a90, sin_a90, color=r.color, type=r.type, robot=r
                ):
                    segments.append(s)
                for w in segments:
                    retval = w.intersects(seg)
    ...
    def process(self, request, sockname, pickleIt=1):
        ...
        if request == "reset":
            ...
            if message[0] == "m":  # "m_t_r" move:translate:rotate
                ...
                if (
                    message[1] in self.assoc[sockname[1]].display
                    and self.assoc[sockname[1]].display[message[1]] != -1
                ):
...
class TkSimulator(tkinter.Toplevel, Simulator):
    def __init__(self, dimensions, offsets, scale, root=None, run=1):
        ...
        self.canvas = tkinter.Canvas(
            self.frame, bg="white", width=self._width, height=self._height
        )
    ...
    def addMouseBindings(self):
        self.canvas.bind(
            "<B1-Motion>", func=lambda event=self: self.dispatch_event(event, "motion")
        )
        self.canvas.bind(
            "<Button-1>", func=lambda event=self: self.dispatch_event(event, "down")
        )
        self.canvas.bind(
            "<ButtonRelease-1>",
            func=lambda event=self: self.dispatch_event(event, "up"),
        )
        self.canvas.bind(
            "<Control-B1-Motion>",
            func=lambda event=self: self.dispatch_event(event, "control-motion"),
        )
        self.canvas.bind(
            "<Control-Button-1>",
            func=lambda event=self: self.dispatch_event(event, "control-down"),
        )
        self.canvas.bind(
            "<Control-ButtonRelease-1>",
            func=lambda event=self: self.dispatch_event(event, "control-up"),
        )
    ...
    def click_b3_up(self, event):
        ...
        self.offset_x = (radius_stop / radius_start) * self.offset_x + (
            1 - (radius_stop / radius_start)
        ) * center[0]
        self.offset_y = (radius_stop / radius_start) * self.offset_y + (
            1 - (radius_stop / radius_start)
        ) * center[1]
        self.redraw()
        ...
        for path in self.trail:
            if self.robots[i].subscribed and self.robots[i].display["trail"] == 1:
                if path[self.trailStart] != None:
                    ...
                    for p in range(
                        self.trailStart, self.trailStart + self.maxTrailSize
                    ):
    ...
    def printDetails(self):
        print(
            "Window: size=(%d,%d), offset=(%d,%d), scale=%f"
            % (
                self.winfo_width(),
                self.winfo_height(),
                self.offset_x,
        ...
        for robot in self.robots:
            print(
                "   %s: pose = (%.2f, %.2f, %.2f)"
                % (robot.name, robot._gx, robot._gy, robot._ga % (2 * math.pi))
            )
    ...
    def drawText(self, x, y, text, fill="black", tag="robot", **args):
        ...
        if not self.tkfont:
            ...
            self.font = tkinter.font.Font(size=-fontPixelHeight)  # -n is n pixels tall
            self.tkfont = tkinter.font.Font(self.canvas, font=self.font)
        ...
        self.canvas.create_polygon(
            points, tag=(tag, "top"), fill="white", outline="black"
        )
        self.canvas.create_text(
            self.scale_x(x) + between + 5,
            self.scale_y(y) - above,
            text=text,
            font=self.font,
            tag=(tag, "top"),
            fill=fill,
    ...
    def addTrail(self, pos, index, robot):
        ...
        if robot.display["trail"] == 1:
            ...
            if xya != None:
                self.drawLine(
                    xya[0], xya[1], robot._gx, robot._gy, robot.color, "trail"
                )
...
class SimRobot:
    def __init__(self, name, x, y, a, boundingBox=[], color="red"):
        ...
        if boundingBox != []:
            self.radius = max(
                max(list(map(abs, boundingBox[0]))), max(list(map(abs, boundingBox[1])))
            )  # meters
        else:
            self.radius = 0.0
    ...
    def additionalSegments(self, x, y, cos_a90, sin_a90, **dict):
        ...
        if self.gripper:
            ...
            w = [
                Segment(xys[0], xys[1], partOf="gripper"),
    ...
    def addBoundingSeg(self, boundingSeg):
        ...
        segradius = max(
            max(list(map(abs, boundingSeg[0]))), max(list(map(abs, boundingSeg[1])))
        )  # meters
        ...
        for d in self.devices:
            ...
            if d.type == "sonar" or d.type == "ir" or d.type == "bumper":
                ...
                for x, y, a in d.geometry:
                    ...
                    dist, hit, obj = self.simulator.castRay(
                        self, gx, gy, -ga, d.maxRange
                    )
                    if hit:
                        self.drawRay(
                            d.type, gx, gy, hit[0], hit[1], self.colorParts[d.type]
                        )
                    else:
                        hx, hy = math.sin(-ga) * d.maxRange, math.cos(-ga) * d.maxRange
                        dist = d.maxRange
                        self.drawRay(
                            d.type, gx, gy, gx + hx, gy + hy, self.colorParts[d.type]
                        )
                    if d.type == "bumper":
                        if dist < d.maxRange:
                            d.scan[i] = 1
                ...
                for (d_x, d_y, d_a) in d.geometry:
                    ...
                    for light in self.simulator.lights:  # for each light source:
                        ...
                        if light.type == "fixed":
                            ...
                            if light.robot == self:
                                continue  # don't read the bulb if it is on self
                            ogx, ogy, oga, brightness = (
                                light.robot._gx,
                                light.robot._gy,
                                light.robot._ga,
                                light.brightness,
                            ...
                            oa90 = oga + PIOVER2
                            x = ogx + (
                                light.x * math.cos(oa90) - light.y * math.sin(oa90)
                            )
                            y = ogy + (
                                light.x * math.sin(oa90) + light.y * math.cos(oa90)
                            )
                ...
                for i in range(2):  # two beams
                    ...
                    dist, hit, obj = self.simulator.castRay(
                        self, gx, gy, -self._ga + PIOVER2, 2 * y, rayType="breakBeam"
                    )
                    if hit:
                        d.scan[i] = 1
                ...
                for i in range(d.width):
                    ...
                    ga = self._ga + a
                    dist, hit, obj = self.simulator.castRay(
                        self, x, y, -ga, ignoreRobot="self", rayType="camera"
                    )
                    if obj != None:
                        if i in [0, d.width - 1]:
                        ...
                        if obj.type == "wall":
                            height = int(
                                min(max((dist ** 2) * d.height / 2.0, 1), d.height / 2)
                            )
                        else:
                            height = int(
                                min(max((dist ** 2) * d.height / 4.0, 1), d.height / 4)
                            )
                        ...
                        if r.type == "puck":  # other robot is a puck
                            if bbintersect:
                                ...
                                if movePucks:
                                    ...
                                    )  # send in random direction, 22 degree
...
class TkRobot(SimRobot):
    ...
    def mouse_event(self, event, command, robot):
        ...
        if command[:8] == "control-":
            self._mouse_xy = x, y
            cx, cy = (
                self.simulator.scale_x(robot._gx),
                self.simulator.scale_y(robot._gy),
            )
            if command == "control-up":
                ...
                self._mouse = 1
                self.simulator.remove("arrow")
                self.simulator.canvas.create_line(
                    cx, cy, x, y, tag="arrow", fill="purple"
                )
        else:
            if command == "up":
                ...
                robot.setPose(
                    x - self._mouse_offset_from_center[0],
...
class TkPuck(TkRobot):
    ...
    def draw(self):
        ...
        if self.display["body"] == 1:
            x1, y1, x2, y2 = (
                (self._gx - self.radius),
                (self._gy - self.radius),
                (self._gx + self.radius),
                (self._gy + self.radius),
            )
            self.simulator.drawOval(
                x1,
                y1,
                x2,
                y2,
                fill=self.color,
                tag="robot-%s" % self.name,
                outline="black",
            )
        if self.display["boundingBox"] == 1 and self.boundingBox != []:
            ...
            self.simulator.drawPolygon(
                xy, tag="robot-%s" % self.name, fill="", outline="purple"
            )
...
class TkPioneer(TkRobot):
    ...
    def draw(self):
        ...
        if self._last_pose == (self._gx, self._gy, self._ga) and (
            ...
            if self.display["speech"] == 1:
                if self.sayText != "":
                    ...
                    # self.simulator.remove("robot-%s" % self.name)
                    self.simulator.drawText(
                        x, y, self.sayText, tag="robot"
                    )  # -%s" % self.name)
            return  # hasn't moved
        self._last_pose = (self._gx, self._gy, self._ga)
        self.simulator.remove("robot-%s" % self.name)
        # Body Polygon, by x and y lists:
        sx = [0.225, 0.15, -0.15, -0.225, -0.225, -0.15, 0.15, 0.225]
        sy = [0.08, 0.175, 0.175, 0.08, -0.08, -0.175, -0.175, -0.08]
        ...
        if self.display["body"] == 1:
            ...
            self.simulator.drawPolygon(
                xy, fill=self.color, tag="robot-%s" % self.name, outline="black"
            )
            bx = [0.14, 0.06, 0.06, 0.14]  # front camera
            by = [-0.06, -0.06, 0.06, 0.06]
            xy = list(
                map(
                    lambda x, y: (
                        self._gx + x * cos_a90 - y * sin_a90,
            ...
            if self.gripper:
                ...
                xs = []
                ys = []
                xs.append(self.gripper.pose[0])
                ys.append(self.gripper.armPosition + 0.01)
                xs.append(self.gripper.pose[0] + self.gripper.armLength)
                ys.append(self.gripper.armPosition + 0.01)
                xs.append(self.gripper.pose[0] + self.gripper.armLength)
                ys.append(self.gripper.armPosition - 0.01)
                xs.append(self.gripper.pose[0])
                ...
                self.simulator.drawPolygon(
                    xy, tag="robot-%s" % self.name, fill="black", outline="black"
                )
                # right arm:
                xs = []
                ys = []
        ...
        if self.display["boundingBox"] == 1:
            ...
            additionalSegments = self.additionalSegments(
                self._gx, self._gy, cos_a90, sin_a90
            )
...
class TkBlimp(TkRobot):
    ...
    def draw(self):
        ...
        self.simulator.drawLine(
            self._gx, self._gy, x, y, tag="robot-%s" % self.name, fill="blue", width=3
        )
...
class PioneerFrontLightSensors(LightSensor):
    def __init__(self):
        ...
        LightSensor.__init__(self, ((0.225, 0.175, 0), (0.225, -0.175, 0)), noise=0.0)
        self.groups = {
            "front-all": (0, 1),
            "all": (0, 1),
            "front": (0, 1),
...
class PioneerFrontDirectionalLightSensors(DirectionalLightSensor):
    def __init__(self):
        # make sure outside of bb!
        DirectionalLightSensor.__init__(
            self,
            ((0.225, 0.175, -30 * PIOVER180), (0.225, -0.175, 30 * PIOVER180)),
            120 * PIOVER180,
            noise=0.0,
        )
        self.groups = {
...
class TkMyro(TkRobot):
    ...
    def draw(self):
        ...
        self.simulator.remove("robot-%s" % self.name)
        # Body Polygon, by x and y lists:
        sx = [0.20, 0.20, -0.10, -0.10]
        sy = [0.15, -0.15, -0.15, 0.15]
        s_x = self.simulator.scale_x
        s_y = self.simulator.scale_y
        a90 = self._ga + PIOVER2  # angle is 90 degrees off for graphics
        cos_a90 = math.cos(a90)
        ...
        if self.display["body"] == 1:
            ...
            for i in range(len(bx)):
                ...
                self.simulator.drawPolygon(
                    xy, tag="robot-%s" % self.name, fill=colors[i]
                )
            # --------------------------------------------------------------------------
            if self.bulb:
                x = self._gx + self.bulb.x * cos_a90 - self.bulb.y * sin_a90
...
class MyroIR(RangeSensor):
    def __init__(self):
        RangeSensor.__init__(
            self,
            "ir",
            geometry=((0.175, 0.13, 45 * PIOVER180), (0.175, -0.13, -45 * PIOVER180)),
            arc=5 * PIOVER180,
            maxRange=0.5,
            noise=0.0,
        )
...
class MyroBumper(RangeSensor):
    def __init__(self):
        RangeSensor.__init__(
            self,
            "bumper",
            geometry=((0.20, 0.0, 80 * PIOVER180), (0.20, 0.0, -80 * PIOVER180)),
            arc=5 * PIOVER180,
            maxRange=0.20,
            noise=0.0,
        )
...
class MyroLightSensors(LightSensor):
    def __init__(self):
        LightSensor.__init__(self, ((0.18, 0.13, 0), (0.18, -0.13, 0)), noise=0.0)
        self.groups = {
            "front-all": (0, 1),
            "all": (0, 1),
            "front": (0, 1),
