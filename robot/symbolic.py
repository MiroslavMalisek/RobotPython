...
class BulbSimDevice(Device):
    ...
    def addWidgets(self, window):
        b = 1.0
        window.addCommand(
            "brightness", "Brightness!", str(b), lambda b: self.setBrightness(float(b))
        )
...
class GripperSimDevice(GripperDevice):
    ...
    def stop(self):
        return self._dev.move("z_gripper_0_stop")

    def store(self):
        return self._dev.move("z_gripper_0_store")

    def deploy(self):
        return self._dev.move("z_gripper_0_deploy")
...
class Simbot(Robot):
    ...
    def init(self, startDevices=1):
        ...
        simDevs = self.getItem("builtinDevices")
        if simDevs:
            self.builtinDevices = simDevs
    ...
    def move(self, message, other=None):
        ...
        retval = None
        if other != None:
            return  # rotate,translate command ignored
        if (
            message == "quit"
            or message == "exit"
            or message == "end"
            or message == "disconnect"
        ):
    ...
    def startDeviceBuiltin(self, name, index=0):
        if name == "simulation":
            ...
            groups = self.move("r_%s_%d" % (name, index))
            return {
                name: DirectionalLightSimDevice(name, index, self, geometry, groups)
            }
        else:
            self.properties.append("%s_%d" % (name, index))
            self.move("s_%s_%d" % (name, index))
    ...
    def move(self, message, other=None):
        ...
        if (
            ...
            try:
                ...
                print("Rob-Error recvfrom")
                retval = ""
            retval = retval.strip()
            try:
                # print("Rob-RECV ",pickle.loads(retval))
                exp = pickle.loads(retval)
            except:
                print("Rob-Error processing message")
                exp = retval
        self.lock.release()
        return exp
