...
import types, random, math, threading
...
class Device(object):
    ...
    def __init__(self, deviceType="unspecified", visible=0, asyncDev=0, sleep=0.01):
    ...
    def __getitem__(self, item):
        ...
        if type(item) == str:
            ...
            if stop < 0 and step > 0:
                stop = len(self) + stop
            return [self.getSensorValue(p) for p in range(start, stop, step)]
        else:
